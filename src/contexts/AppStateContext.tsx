...
import { userReducer, safeMerge, toYMD, addDays, recalculateStreaks, timePresets } from './userReducer';
...
export const AppStateProvider = ({ children }: { children: React.ReactNode }) => {
      // Move hooks inside the component
        ...
        const [appState, setAppState] = React.useState<AppState>(() => ({ ...defaultUserState, view: 'cockpit' } as AppState));
    ...
    // given to the explicit skipDev flag when present.
        ...
        if (e2eForceView) {
            ...
            // Back-compat: Accept 'command-center' as a legacy alias for 'cockpit'
            const normalizedE2EView = e2eForceView === 'command-center' ? 'cockpit' : e2eForceView;
            if (normalizedE2EView === 'cockpit') seededState.dashboardType = 'william';
        ...
        try {
            const isExplicitE2ESeed = typeof window !== 'undefined' && (
  ...
  // Create a dispatch function that writes to the database
        ...
        React.useEffect(() => {
            try {
                ...
                if ((e2eInit && e2eInit.dashboardType === 'william') || e2eForceGM) {
                    next = { ...next, view: 'game-master-dashboard', dashboardType: 'william' } as AppState;
                }
    ...
    const isTestMode = typeof window !== 'undefined' && (!!(window as any).__WONKY_E2E_TEST_MODE__ || (import.meta as any).env?.VITE_PLAYWRIGHT_ACCELERATE === 'true');
        const getPersonaDisplayName = (personaKey: string) => {
            try {
                if (!appState) return personaKey;
                if (!appState.personaOverrides) return personaKey;
                return appState.personaOverrides[personaKey] || personaKey;
            } catch (e) { return personaKey; }
        };
        const contextValue: AppContextType = { authUser, appState, dispatch: dispatchWrapper, isTestMode, getPersonaDisplayName };
