...
const KonamiModal: React.FC<{ onClose: () => void }> = ({ onClose }) => (
  <div className="konami-modal-bg">
    <div>
      <span role="img" aria-label="sprout" className="konami-modal-emoji">🌱</span>
      <h2>Level Up Unlocked!</h2>
      <p>"You found the secret! Keep growing, keep coding."</p>
      <button className="konami-modal-btn" onClick={onClose}>Close</button>
    </div>
  </div>
);
...
import UserSopView from '../../UserSopView';
...
const AppContent = () => {
  ...
  if (typeof window !== 'undefined' && appState) {
    ...
    // Only verbose debug output in development to keep console clean in production
  ...
  return (
    <div className="min-h-screen bg-sanctuary-bg text-sanctuary-text-main flex flex-col">
      ...
      {/* The Airlock — overlays the app until release */}
      <ContextAirlock />
