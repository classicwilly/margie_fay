...
import { getGrandmaAdvice, getGrandpaAdvice, getBobAdvice, getMargeAdvice } from '../services/geminiService';
...
const PERSONA_UI = {
  grandma: { key: 'grandma', icon: '👵', helper: getGrandmaAdvice, color: 'text-accent-pink', role: 'Structural Fix' },
  grandpa: { key: 'grandpa', icon: '🛠️', helper: getGrandpaAdvice, color: 'text-accent-teal', role: 'Systems Integrity' },
  bob: { key: 'bob', icon: '⚡️', helper: getBobAdvice, color: 'text-yellow-400', role: 'Kinetic Coach' },
  marge: { key: 'marge', icon: '💖', helper: getMargeAdvice, color: 'text-purple-400', role: 'Empathetic Planner' },
};
...
export const GrandmaHelper: React.FC = () => {
  ...
  const handleAsk = async (userQuery: string) => {
    if (isLoading || userQuery.trim() === '') return;
    ...
    if (redacted) setRedactedWarning(true);
    ...
    try {
      ...
      setResponse(`Communication array offline. Please check network. ${activePersona.key === 'grandma' ? 'Love, Grandma.' : '—' + activePersona.key + '.'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        <div className={`text-4xl ${activePersona.color}`} data-testid="persona-avatar">{activePersona.icon}</div>
        <div className="flex-1">
          <h3 className={`text-xl font-bold uppercase ${activePersona.color}`} data-testid="persona-display-name">
