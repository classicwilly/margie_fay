...
const E2EDebugView = () => {
  ...
  try {
    ...
    appStateVal = typeof window !== 'undefined' ? (window as any).appState : undefined;
  }
  ...
  const dashboard = appStateVal?.dashboardType || (typeof window !== 'undefined' ? (window as any).__WONKY_TEST_INITIALIZE__?.dashboardType : 'unknown') || 'unknown';
  try {
  ...
  return (
    <div data-testid="e2e-runtime-view" data-e2e-view={view} data-e2e-dashboard={dashboard} aria-live="polite" className="e2e-debug">
      ...
      <div className="e2e-meta">
        <small>{typeof window !== 'undefined' ? `sticky:${!!(window as any).__WONKY_TEST_STICKY_VIEW__} allowDB:${(window as any).__WONKY_TEST_CAN_UPDATE_DB__ ? (window as any).__WONKY_TEST_CAN_UPDATE_DB__() : 'n/a'}` : ''}</small>
      </div>
    </div>
  );
};
