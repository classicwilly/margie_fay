...
const ContextAirlock: React.FC = () => {
  ...
  if (!isAirlockActive) return null;
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-background-dark/95 backdrop-blur-xl transition-colors duration-500">
      {/* Protocol Header */}
      <h2 className={`text-xl font-mono font-bold tracking-[0.2em] mb-8 uppercase ${isSourMode ? 'text-sour-accent' : 'text-text-muted'}`}>
      ...
      <div className="relative flex items-center justify-center mb-12">
        {/* Outer Ring */}
        <div className={`absolute inset-0 rounded-full border-2 opacity-20 animate-pulse-slow scale-150 ${isSourMode ? 'border-sour-accent' : 'border-accent-teal'}`}></div>
      ...
      {/* Status Text */}
      <p className={`mb-12 font-mono text-sm uppercase tracking-widest animate-pulse ${isSourMode ? 'text-sour-accent' : 'text-accent-pink'}`}>
      ...
      <div className="flex flex-col gap-4 items-center w-full max-w-xs">
        <button
          ...
          disabled={!canBypass}
          className={
            `w-full py-4 px-6 border-2 font-mono font-bold text-sm uppercase tracking-wider transition-all duration-300 rounded-lg
            ${canBypass 
              ? (isSourMode 
          ...
          `}
        >
          {canBypass ? "⚠️ Override Protocol" : `Lockdown Active (${BYPASS_DELAY_SECONDS - elapsed}s)`}
        </button>
