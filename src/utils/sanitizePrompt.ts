...
const PII_REGEXES = [
    { pattern: /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi, replacement: '[REDACTED EMAIL]' }, // Email
    { pattern: /(\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4})/g, replacement: '[REDACTED PHONE]' }, // Phone (common US formats)
    { pattern: /\b\d{3}[-.\s]?\d{2}[-.\s]?\d{4}\b/g, replacement: '[REDACTED SSN]' }, // SSN/Tax ID
 ...
 */
export function sanitizePrompt(query: string): { cleanedQuery: string, redacted: boolean } {
