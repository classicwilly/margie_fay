...
function exec(cmd) {
  try {
    return childProcess.execSync(cmd, { cwd: repoRoot, stdio: 'pipe' }).toString().trim();
  } catch (err) {
    return null;
  }
}
...
const manifestPath = args.manifest || path.join('scripts', 'refactor', 'rename-manifest.json');
...
for (const entry of manifest) {
  ...
  log(`Processing: ${entry.from} -> ${entry.to} (oldExists=${oldExists}, newExists=${newExists})`);

  // If new path exists, we will just create a shim at old path that re-exports from new, and optionally remove old
  if (newExists) {
    log('Destination already exists; creating shim and backing up original (if present)');
    ...
    const shimSrc = `// DEPRECATED SHIM: ${path.relative(repoRoot, entry.from)} -> ${path.relative(repoRoot, entry.to)}\n` +
    ...
    renameResults.push({ from: oldPath, to: newPath, shimCreated: true, backedUp: oldExists });
    continue;
  }
  ...
  if (oldExists) {
    ...
    fs.mkdirSync(path.dirname(oldPath), { recursive: true });
    fs.writeFileSync(oldPath, shimSrc, 'utf8');
    renameResults.push({ from: oldPath, to: newPath, moved: true, shimCreated: true });
  } else {
    ...
    fs.mkdirSync(path.dirname(oldPath), { recursive: true });
    fs.writeFileSync(oldPath, shimSrc, 'utf8');
    renameResults.push({ from: oldPath, to: newPath, shimCreated: true, createdOnly: true });
  }
  ...
  for (const sf of allSourceFiles) {
    ...
    sf.getImportDeclarations().forEach((decl) => {
      ...
      if (resolved === fromNoExt || resolved === fromNoExt + '.tsx' || resolved === fromNoExt + '.ts') {
...
if (!gitCheckout) console.error('Failed to create branch. Are you inside a Git repo?');
else log('Created branch', branchName);

const gitAdd = exec('git add -A');
const gitCommit = exec(`git commit -m "chore(refactor): rename files and add shims (${branchName})"`);
...
if (!pushResult) console.warn('Push failed or no remote configured: run `git push -u origin', branchName, '` manually.');
else console.log('Pushed branch to origin/', branchName);

console.log('safe-rename: Done. Please open a pull request for branch:', branchName);
