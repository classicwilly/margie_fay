...
(async () => {
  ...
  await page.addInitScript(() => {
    try {
      window.__E2E_FORCE_VIEW__ = 'cockpit';
      window.__WONKY_TEST_INITIALIZE__ = { dashboardType: 'william', view: 'cockpit', initialSetupComplete: true };
  ...
  const found = await page.$('h1[data-testid="cockpit-title"]');
  if (found) {
