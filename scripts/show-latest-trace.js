...
const traceDirs = [
  ...
  path.resolve("./playwright-report/data"),
];
...
try {
  ...
  if (process.platform === "win32") {
    spawn("cmd", ["/c", "npx", "playwright", "show-trace", latest], {
      stdio: "inherit",
      detached: true,
    });
  } else {
    spawn("npx", ["playwright", "show-trace", latest], {
      stdio: "inherit",
      detached: true,
    });
  }
...
if (fs.existsSync(verifyScript)) {
  try {
    ...
    execFileSync("node", [verifyScript], {
      stdio: "inherit",
      env: process.env,
    });
  } catch (err) {
    console.error(
      "verify-dashboard script returned non-zero or failed:",
      err.message || err,
    );
