...
(async () => {
  ...
  let h1 = null;
  try {
    await page.waitForSelector('h1[data-testid="command-center-title"]', { timeout: 10000 });
    h1 = await page.$('h1[data-testid="command-center-title"]');
  } catch (e) {
    // Not found
  }
  ...
  if (h1) {
    ...
    console.warn('H1 not found (data-testid=command-center-title)');
  }
