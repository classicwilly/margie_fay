...
async function main() {
  ...
  if (!key || !key.client_email) {
    console.log('Please provide GOOGLE_SERVICE_ACCOUNT_KEY_JSON in env variables.');
    process.exit(1);
  }
  ...
  if (spreadsheetId) {
    await sheets.spreadsheets.values.append({ spreadsheetId, range: 'A1', valueInputOption: 'USER_ENTERED', resource: { values: [['timestamp', new Date().toISOString(), 'sample', 'automated']] } });
    console.log('Appended a row to Sheets');
  } else {
    console.log('No GOOGLE_SAMPLE_SPREADSHEET_ID provided, skipping Sheets append.');
  }

  // Calendar example: create an event
  const calendar = google.calendar({ version: 'v3', auth });
  const cal = process.env.GOOGLE_SAMPLE_CALENDAR_ID || 'primary';
  const event = await calendar.events.insert({ calendarId: cal, requestBody: { summary: 'Demo: Test event', start: { dateTime: new Date().toISOString() }, end: { dateTime: new Date(Date.now() + 30 * 60 * 1000).toISOString() } } });
  console.log('Created calendar event id:', event.data.id);

  // Drive: create a simple doc
  const drive = google.drive({ version: 'v3', auth });
  const res = await drive.files.create({ requestBody: { name: 'Wonky-SOP-Demo', mimeType: 'application/vnd.google-apps.document' }, fields: 'id' });
