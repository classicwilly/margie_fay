...
export function useLiveChatFunctions(
    ...
    const handleSynthesizeSop = useCallback(async (topic, entries, cardId) => {
        ...
        try {
            ...
            const draft = response.json || (response.text ? JSON.parse(response.text) : null);
        ...
        return {
            ...
            'captureAndProcessThought': async (args) => {
                ...
                (async () => {
                    try {
                        const { generate } = useSafeAI();
                        const processResponse = await generate(`Summarize this thought into a title, structured content, and tags. Thought: "${args.thought}"`, {
                            model: 'gemini-2.5-flash',
                            responseMimeType: 'application/json',
                            responseSchema: { type: Type.OBJECT, properties: { title: { type: Type.STRING }, content: { type: Type.STRING }, tags: { type: Type.ARRAY, items: { type: Type.STRING } } } },
                            timeoutMs: 20000
                        });
                ...
                (async () => {
                    ...
                    try {
                        ...
                        const response = await generate(prompt, {
                            model: 'gemini-2.5-flash',
                            responseMimeType: 'application/json',
                            responseSchema: { type: Type.OBJECT, properties: { relatedIds: { type: Type.ARRAY, items: { type: Type.STRING } } } },
                            timeoutMs: 20000
                        });
