...
router.get('/demo', requireAuth, async (req, res) => {
  try {
    ...
    const userTokens = sessionId && cache && cache.get ? await cache.get(`google_tokens:${sessionId}`) : null;
    let auth;
    if (userTokens && process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
      const oauth2 = new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, process.env.GOOGLE_OAUTH_REDIRECT_URI || `http://localhost:${process.env.PORT || 8080}/api/google/oauth2callback`);
      ...
      return res.json({ demo: true, msg: 'No Google credentials available — returning a mock sample.' });
    }
    ...
    if (userTokens) {
      try {
        ...
        return res.json({ demo: true, mode: 'user', events: eventsResp.data.items || [], profile: profileData });
      } catch (e) {
    ...
    try { req.app?.get('auditLogger')?.('google.demo', { mode: 'service' }); } catch (e) { /* noop */ }
