...
app.post('/api/gemini', async (req, res) => {
  try {
    ...
    const cacheKey = crypto.createHash('sha256').update(`${model}|${sanitized}|${systemInstruction}|${maxTokens}`).digest('hex');
    ...
    try { if (cache && cache.set) cache.set(cacheKey, json); } catch (e) { /* noop */ }
...
const server = app.listen(port, () => {
  ...
  if (!process.env.GEMINI_API_KEY && process.env.NODE_ENV === 'production' && !process.env.DEV_ALLOW_MISSING_GEMINI_KEY) {
    console.error('GEMINI_API_KEY is not set - the Gemini proxy will not function. Set GEMINI_API_KEY in Secrets/Env.');
    console.error('Failing fast for production safety. Use DEV_ALLOW_MISSING_GEMINI_KEY=true to override in non-production deploys.');
    process.exit(1);
...
app.get('*', (req, res) => {
  if (req.path.startsWith('/api/') || req.path.startsWith('/metrics') || req.path.startsWith('/ai_stub_event') ) {
...
const shutdown = async (signal) => {
  ...
  } catch (e) { console.warn('Error closing cache', e?.message || e); }
  server.close(() => { console.log('HTTP server closed'); process.exit(0); });
  setTimeout(() => { console.warn('Force exit'); process.exit(1); }, 10000);
};
