                            ...
                            <div className="space-y-3">
                                {Array.isArray(projects) && projects.map((project: any) => (
                                    <div key={project.id} className="card-base">
                                        ...
                                        <p className="text-xs text-text-light mt-1">Completed & Archived</p>
                                    </div>
                                ))}
                            </div>
                        </ContentCard>
                    ))}
                    {archivedProjects.length === 0 && <p className="text-center text-text-light">No projects have been completed and archived yet.</p>}
                </div>
            )}
                ...
                <div>
                    ...
                    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                        {filteredKnowledge.map((entry: any) => (
                            <details key={entry.id} className="card-base border border-surface-700 group">
                                ...
                                <div className="pt-2 mt-2 border-t border-surface-600">
                                    ...
                                    <div className="flex justify-end">
                                        <button onClick={() => handleUnarchiveKnowledge(entry.id)} className="btn-primary text-xs font-semibold">
                                            Restore to Vault
                                        </button>
                                    </div>
                                </div>
                         ...
                         {archivedKnowledge.length === 0 && <p className="text-center text-text-light">No knowledge entries have been archived yet.</p>}
                    </div>
                </div>
            )}
        </div>
