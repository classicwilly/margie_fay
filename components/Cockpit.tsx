...
const Cockpit: React.FC = () => {
    ...
    const handleTestAirlock = (e: React.MouseEvent) => {
        ...
        try { dispatch?.({ type: 'SET_SAVED_CONTEXT', payload: { view: 'cockpit', dashboardType: 'william' } }); } catch (e) { /* ignore */ }
        try { setAirlockActive(true); } catch (e) { /* ignore */ }
    };
    ...
    <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
            <div className="mb-4 md:mb-0">
                <p className="font-mono text-sm text-accent-teal">PROTOCOL 11-22-44 // INTEGRITY CHECK</p>
                <h1 data-testid="cockpit-title" className="text-5xl font-extrabold text-accent-teal mt-2">DAY <span className="text-text-light">36,525</span></h1>
            ...
            <div className="flex flex-col items-end">
                <ContentCard showHeader={false} className="mb-4 max-w-xs w-full">
                    <h2 className="text-md font-semibold text-accent-teal">FIELD REPORT</h2>
                ...
                <div className="card-base px-4 py-2 rounded-full flex items-center space-x-2">
                    ...
                    <span className="text-lg text-accent-teal font-bold">Deep Focus</span>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-zap text-accent-teal"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
                </div>
            </div>
        </div>
        </DeepFocusContainer>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <ContentCard title="Workspace Launchpad">
                ...
                <div className="mt-4 flex gap-2">
                    <Button data-testid="test-airlock-btn" variant="danger" className="font-bold" onClick={handleTestAirlock}>🛑 Test Airlock Protocol</Button>
                    <Button variant="secondary" className="font-bold" onClick={() => { dispatch({ type: 'SET_CONTEXT_RESTORE_MODAL_OPEN', payload: false }); dispatch({ type: 'SET_VIEW', payload: 'bio-hacks' }); }}>💊 The Apothecary</Button>
                </div>
            </ContentCard>
            <CriticalTasks />
        </div>
