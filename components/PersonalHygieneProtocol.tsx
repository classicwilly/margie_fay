...
const PersonalHygieneProtocol: React.FC = () => {
  const protocolData = ALL_CHECKLIST_DATA.filter(
    (s) => s.sourceDocument === "Personal Hygiene Protocol",
  );
    ...
    <div>
      <header className="text-center mb-10">
        ...
        <p className="text-lg text-text-light text-opacity-80">
          A tiered hygiene system for neurodivergent brains. Match hygiene to
          your capacity.
        </p>
      </header>
      ...
      <div className="space-y-6">
          ...
          <section key={section.id} id={section.id}>
            <ContentCard title={section.title}>
              ...
              {section.id.startsWith("php-t") && (
                <p className="mt-4 text-lg font-semibold text-accent-blue pl-4">
                  {section.validation}
                </p>
              )}
            </ContentCard>
        ...
        >
          <p className="text-xl font-bold text-accent-blue">
            Your hygiene is not your worth. Skipped shower ≠ bad person. This
            protocol exists to remove shame and provide structure. Use it. Adapt
            it. Survive.
          </p>
        </footer>
      </div>
