...
const MemorialsPage: React.FC = () => {
  ...
  useEffect(() => { saveMemorials(items); }, [items]);
  ...
  const onAddMemory = (id: string) => {
    ...
    setItems(prev => prev.map(p => p.id === id ? { ...p, excerpt: `${p.excerpt}\n\n${new Date().toLocaleDateString()}: ${note}` } : p));
  };
  ...
  return (
    <main className="container mx-auto p-4">
      {celebrate && <div className="p-4 mb-6 bg-accent-teal/10 rounded shadow-neon-sm text-center font-bold">Celebrating a loved one today 💖</div>}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map(item => (
          <div key={item.id}>
            <MemorialCard entry={item} onCelebrate={() => onCelebrate(item.id)} onAddMemory={() => onAddMemory(item.id)} />
          </div>
        ))}
      </div>
    </main>
