        ...
        <ContentCard title={<><GoogleEmoji symbol={'🎁'} size={20} className="mr-2" />Dopamine Mining</>}>
            ...
            <div className="space-y-3">
                {REWARD_TIERS.map(tier => {
                    ...
                    let button;
                    if (isAcknowledged) {
                        button = <Button disabled variant="primary" size="sm">✓ Fulfilled!</Button>;
                    } else if (isRedeemed) {
                        button = <Button disabled variant="secondary" size="sm">Pending...</Button>;
                    } else if (isUnlocked) {
                        button = <Button data-testid={`redeem-reward-btn-${tier.threshold}`} onClick={() => handleRedeem(tier.threshold)} variant="primary" size="sm">Redeem</Button>;
                    } else {
                        button = <Button disabled variant="secondary" size="sm">Locked</Button>;
                    }
