    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8">
            <header className="flex flex-col sm:flex-row justify-between sm:items-center text-center sm:text-left mb-12 gap-6">
                <div>
                    <h1 className="text-5xl font-extrabold text-gradient-accent drop-shadow-lg mb-2">Operations Control</h1>
                    <p className="text-lg text-text-light text-opacity-80 font-medium">Systems Online. Frequency Tuned. Welcome back, Pilot.</p>
                </div>
                {appState.isModMode && (
                    <Button onClick={handleCustomizeDashboard} variant="primary" size="sm" className="btn-neon animate-neon" aria-label="Customize Dashboard Modules">Customize Dashboard</Button>
                )}
            </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="lg:col-span-1 space-y-8">
                        <div className="card-base shadow-neon border-neon-green">
                            <h2 className="font-bold text-neon-green text-xl mb-4 text-center animate-glow">System Vitals</h2>
                            <div className="grid grid-cols-3 gap-4 text-center">
                                <div>
                                    <p className={`text-4xl font-bold text-neon-blue animate-neon`}>{score}</p>
                                    <p className="text-xs font-semibold text-text-light text-opacity-70">Health</p>
                                </div>
                                <div>
                                    <p className="text-4xl">🧠</p>
                                    <p className="text-xs font-semibold text-neon-yellow animate-neon">{statusMood || 'N/A'}</p>
                                </div>
                                <div>
                                    <p className="text-4xl">🔌</p>
                                    <p className="text-xs font-semibold text-neon-pink animate-neon">{statusEnergy || 'N/A'}</p>
                                </div>
                            </div>
                        </div>
                        <DashboardLauncher />
