...
const AchievementTracker: React.FC = () => {
    ...
    const collectedIds = appState && appState.collectedAchievements ? Object.keys(appState.collectedAchievements) : [];
                    ...
                    <div className="mb-4">
                        {nextTier ? (
                            <>
                                ...
                                >
                                    <span className="sr-only">Progress: {Math.round(progressPercentage)}%</span>
                                    <div ref={progressRef} className="bg-gradient-to-r from-accent-blue to-accent-green h-3 rounded-full transition-all duration-500 progress-fill" />
                                </div>
                            </>
                        ) : (
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                            {ALL_ACHIEVEMENTS.map(ach => (
                                <Achievement 
                                    ...
                                    emoji={ach.emoji}
                                    label={ach.label}
                                    collected={Boolean(appState.collectedAchievements && appState.collectedAchievements[ach.id])}
                                />
                            ))}
                        </div>
                    </div>
