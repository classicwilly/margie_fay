...
const Celebration: React.FC<{ active?: boolean }> = ({ active=false }) => {
  ...
  const choices = {
    left: ['left-5','left-15','left-25','left-35','left-45','left-55','left-65','left-75','left-85'],
  ...
  return (
    <div className="confetti" aria-hidden>
      {hearts.map((h,i)=> {
        const left = choices.left[Math.floor(Math.random()*choices.left.length)];
        const top = choices.top[Math.floor(Math.random()*choices.top.length)];
        const size = choices.size[Math.floor(Math.random()*choices.size.length)];
        const delay = choices.delay[Math.floor(Math.random()*choices.delay.length)];
