...
const WeeklyReview: FC = () => {
  ...
  const weeklyReviewData = ALL_CHECKLIST_DATA.filter(
    ...
    <div className="container mx-auto px-4 py-8">
      <header className="text-center mb-10">
        ...
        <p className="text-lg text-text-light text-opacity-80">
          Use this checklist every Friday 3-4 PM (Solo Mode) or Sunday 8:30-9:30
          PM (Family Mode). Progress saves automatically.
        </p>
      </header>
            ...
            {section.items && (
              <ul className="list-none">
                {section.items.map((item) => {
                  ...
                  const microSteps =
                    microStepsMode && fullLabel.includes(":")
                      ? fullLabel.split(". ").filter((s) => s.trim() !== "")
                      : [fullLabel];
                  return microSteps.map((step, idx) => (
                    <ChecklistItem
                      key={`${item.id}-${idx}`}
                      id={item.id}
                      ...
                      large={item.large}
                    >
                      {microStepsMode && microSteps.length > 1
                        ? `• ${step}`
                        : step}
                    </ChecklistItem>
                  ));
            ...
            {section.subSections &&
              section.subSections.map((subSection) => (
                <ContentCard
                  ...
                  className="mb-4 bg-surface-800 rounded-lg"
                >
                  <ul className="list-none">
