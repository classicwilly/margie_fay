...
const FamilyStructureModeProtocol: FC = () => {
  const protocolData = ALL_CHECKLIST_DATA.filter(
    (s) => s.sourceDocument === "Family Structure Mode Protocol",
  );
    ...
    <div>
      <header className="text-center mb-10">
        ...
        <p className="text-lg text-text-light text-opacity-90 mt-2 font-semibold">
          Purpose: Shared structure, low-stimulation environment, and
          high-vigilance parenting.
        </p>
      </header>
            ...
            <ContentCard title={section.title}>
              ...
              {section.subSections?.map((subSection) => (
                <ContentCard
                  ...
                  titleClassName="text-yellow-400 text-xl"
                  className="border-yellow-500 mt-4"
                >
                  {subSection.description && (
                    <p className="text-lg text-text-light text-opacity-90 mb-6">
                      {subSection.description}
                    </p>
                  )}
                  <ul className="list-none space-y-2 text-lg">
