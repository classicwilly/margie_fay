...
const LifeMaintenanceProtocol: FC = () => {
  const protocolData = ALL_CHECKLIST_DATA.filter(
    (s) => s.sourceDocument === "Life Maintenance Protocol",
  );
    ...
    <div>
      <header className="text-center mb-10">
        ...
        <p className="text-lg text-text-light text-opacity-80">
          The unglamorous survival tasks that collapse when executive function
          fails: House cleaning, groceries, and personal hygiene.
        </p>
      </header>
      ...
      <div className="space-y-6">
        {protocolData.map((section) => (
          <section key={section.id} id={section.id}>
            <ContentCard title={section.title}>
              {section.description && (
                <p className="text-lg text-text-light text-opacity-90 font-semibold mb-4">
                  {section.description}
                </p>
              )}
              {section.items && (
              ...
              {section.subSections?.map((subSection) => (
                ...
                >
                  {subSection.description && (
                    <p className="text-md text-text-light text-opacity-80 mb-4 pl-4">
                      {subSection.description}
                    </p>
                  )}
                  {subSection.items && (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                      {subSection.subSections.map((grocerySection) => (
                        <ContentCard
                          ...
                          title={grocerySection.title}
                          titleClassName="text-accent-blue text-lg"
                        >
                          <ul className="list-none text-lg">
                            {grocerySection.items?.map((item) => (
                              <ChecklistItem key={item.id} id={item.id}>
                                {item.label}
                              </ChecklistItem>
                            ))}
        ...
        >
          <p className="text-xl font-bold text-accent-blue">
            These tasks do not define your worth. This SOP exists to remove
            shame and provide mechanical steps. Use it. Adapt it. Survive.
          </p>
        </footer>
      </div>
    </div>
