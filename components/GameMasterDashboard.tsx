            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Create Quest Form */}
                 <details className="space-y-3 card-base border border-surface-700">
                    ...
                    <form onSubmit={handleCreateQuest} className="space-y-3 pt-3 border-t border-surface-700">
                        <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Quest Title" className="w-full p-2 card-base border border-surface-600" required />
                        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Description" rows={3} className="w-full p-2 card-base border border-surface-600" required />
                        <div>
                            <h4 className="text-sm font-semibold mb-1">Steps:</h4>
                            <div className="space-y-2">
                                {steps.map((step, index) => (
                                    <div key={index} className="flex items-center gap-2">
                                        <input type="text" value={step} onChange={e => handleStepChange(index, e.target.value)} className="flex-grow p-2 card-base border border-surface-600" placeholder={`Step ${index + 1}`} />
                                        {steps.length > 1 && <button type="button" onClick={() => handleRemoveStep(index)} className="btn-primary p-1">&times;</button>}
                                    </div>
                                ))}
                            </div>
                            <button type="button" onClick={handleAddStep} className="mt-2 btn-primary text-xs">+ Add Step</button>
                        </div>
                        <div className="flex gap-2">
                            <select value={gemRewardId} onChange={e => setGemRewardId(e.target.value)} className="w-1/2 p-2 card-base border border-surface-600">
                                {questRewardGems.map(gem => <option key={gem.id} value={gem.id}>{gem.label}</option>)}
                            </select>
                            <select value={assignedTo} onChange={e => setAssignedTo(e.target.value)} className="w-1/2 p-2 card-base border border-surface-600">
                                <option value="both">Both Kids</option>
                                <option value="willow">Willow</option>
                                <option value="sebastian">Sebastian</option>
                            </select>
                        </div>
                        <button type="submit" className="w-full btn-primary font-bold">Create Quest</button>
                    </form>
                </details>
                ...
                <div className="space-y-4">
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                            {questsInReview.map(q => (
                                <div key={q.id} className="card-base">
                                    ...
                                    <button onClick={() => handleApproveQuest(q.id)} className="btn-primary text-xs">Approve & Grant Reward</button>
                                </div>
                            ))}
                        </div>
                    </div>
                     <div>
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                             {activeQuests.map(q => (
                                <div key={q.id} className="card-base flex justify-between items-center">
                                    <p className="font-semibold">{q.title} ({q.assignedTo})</p>
                                    <button onClick={() => handleDeleteQuest(q.id)} className="btn-primary text-xs">Delete</button>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                        {pendingRedemptions.map(({ persona, threshold, tier }) => {
                            ...
                            const redemptionId = `${persona}-${threshold}`;
                            return (
                                <div key={redemptionId} className="card-base border border-yellow-700">
                                    ...
                                    />
                                    <button onClick={() => handleAcknowledge(persona, threshold, tier.title)} className="w-full btn-primary font-bold" data-testid={`mark-fulfilled-${redemptionId}`}>Mark as Fulfilled</button>
                                </div>
                            );
                        })}
                        {pendingRedemptions.length === 0 && <p className="text-sm text-text-light">No rewards waiting for fulfillment.</p>}
                    </div>
                </div>
                <div>
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                        {fulfillmentLog.map(item => (
                            <div key={item.id} className="card-base text-sm">
                                ...
                                <p className="text-xs text-text-light">Fulfilled on {new Date(item.fulfilledAt).toLocaleDateString()}</p>
