...
test("basic AI flow from UI placeholder to telemetry @smoke", async ({
  page,
  storageKey,
}) => {
  ...
  page.on("requestfailed", (r) =>
    console.log("PAGE EVENT: requestfailed", r.url(), r.failure()?.errorText),
  );
  page.on("request", (r) =>
    console.log("PAGE EVENT: request", r.method(), r.url()),
  );
  ...
  await page.addInitScript((key) => {
    ...
    try {
      window.localStorage.setItem(
        key as string,
        JSON.stringify({ initialSetupComplete: true }),
      );
    } catch (e) {
      /* ignore */
    }
  ...
  // Now navigate to the cockpit view
  ...
  const url =
    process.env.PLAYWRIGHT_AI_STUB === "true" ? "/?use_ai_proxy=true" : "/";
  // Ensure the entire DOM is settled after navigation and for the main title to be visible
  await page.waitForLoadState("networkidle");
  await page
    .getByRole("heading", { name: /Workshop|Profile|Stack|DAY/i })
    .first()
  ...
  await page.evaluate(() =>
    console.log(
      "PLAYWRIGHT_AI_STUB flag:",
      (window as any).__PLAYWRIGHT_AI_STUB__,
    ),
  );
  ...
  await page.screenshot({
    path: "tests/e2e/debug/ai_flow_after_load.png",
    fullPage: true,
  });
  ...
  if (await commandCenterNav.count()) {
    ...
    if (!(await systemBtn.count()))
      systemBtn = page.getByRole("button", { name: "System" });
    await systemBtn.click();
    await page.waitForTimeout(150);
    await page.getByRole("menu").waitFor({ state: "visible", timeout: 5000 });
    await page.waitForTimeout(100);
    await retryClick(page.getByRole("menuitem", { name: "The Workshop" }), {
      tries: 3,
    });
  }
  ...
  await retryClick(askBtn, {
    tries: 4,
    interval: 300,
    clickOptions: { timeout: 10000 },
  });
  // Wait for the backend AI call to complete for determinism
  try {
    await page.waitForResponse(
      (resp) => resp.url().includes("/aiProxy") && resp.status() === 200,
      { timeout: 20000 },
    );
  } catch (err) {
    // If the backend didn't respond, allow the UI to fall back to the non-AI message
    console.warn(
      "AI backend did not respond within timeout — falling back on UI check",
    );
  }
  ...
  try {
    await expect(page.getByTestId("pii-warning-title")).toBeVisible({
      timeout: 5000,
    });
    // Prefer data-testid for stable selection
    if (
      page.getByTestId &&
      (await page.getByTestId("pii-send-anyway").count())
    ) {
  ...
  try {
    await expect(page.getByTestId("ai-consent-title")).toBeVisible({
      timeout: 5000,
    });
    // Click by test id when available for localization-safe selection
    if (
      page.getByTestId &&
      (await page.getByTestId("ai-consent-acknowledge").count())
    ) {
      await page.getByTestId("ai-consent-acknowledge").click();
    } else {
      await page
        .getByRole("button", { name: /Acknowledge & Proceed/i })
        .click();
    }
  } catch (err) {
  ...
  console.log(
    "PAGE LOG: Axe violations ids",
    result.violations.map((v) => v.id),
  );
  console.log(
    "PAGE LOG: Axe violation impacts",
    result.violations.map((v) => v.impact),
  );
  ...
  if ((await dialog.count()) > 0) {
    ...
    await page.screenshot({
      path: "tests/e2e/debug/ai_flow_no_modal.png",
      fullPage: true,
    });
  }
});
