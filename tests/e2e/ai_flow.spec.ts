...
test('basic AI flow from UI placeholder to telemetry @smoke', async ({ page, storageKey }) => {
  ...
  await page.getByRole('heading', { name: /Cockpit|Profile|Stack|DAY/i }).first().waitFor({ state: 'visible', timeout: 20000 });
  ...
  let commandCenterNav = page.getByTestId('nav-cockpit');
  ...
  await expect(page.getByTestId('cockpit-title')).toBeVisible({ timeout: 10000 });
  ...
  await retryClick(askBtn, { tries: 4, interval: 300, clickOptions: { timeout: 10000 } });
