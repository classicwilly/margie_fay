...
test('debug: capture wonky-last-error via localStorage', async ({ page }) => {
  ...
  const lastErrorDomText = await page.locator('[data-testid="wonky-last-error"]').allInnerTexts();
  console.error('DEBUG_WONKY_LAST_ERROR_DOM_TEXT:', lastErrorDomText.join('\n'));
