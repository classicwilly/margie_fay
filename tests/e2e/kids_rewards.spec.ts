...
test.skip("Kids corner and reward tiers visible @smoke", async ({ page }) => {
  ...
  const shouldTrace =
    !!process.env.WONKY_E2E_TRACE_TESTS ||
    (!!process.env.CI && testInfo.retry > 0);
  if (shouldTrace)
    ...
    try {
      ...
      try {
        window.localStorage.setItem(
          "wonky_flags",
          JSON.stringify({ aiEnabled: false }),
        );
      } catch (e) {
        /* ignore */
      }
  ...
  try {
    const consoleMsg = await page.waitForEvent("console", {
      timeout: 2000,
      predicate: (m) =>
        m.text().includes("WONKY_E2E_ERROR") ||
        m.text().includes("WONKY_GLOBAL_ERROR_ONERROR"),
    });
  ...
  try {
    ...
    console.warn("WONKY_SEEDED_STATE_RAW failed", e);
  }
  ...
  try {
    ...
    console.warn("WONKY_SEEDED_MODULES failed", e);
  }
  ...
  try {
    ...
    const lastErrRaw = await page.evaluate(() =>
    ...
    if (domError && domError.length > 0) {
      throw new Error(
        `E2E Captured error from ErrorBoundary: ${domError.join("\n")}`,
      );
    }
    ...
    console.warn("Could not read WONKY_LAST_ERROR from page", e);
  }
  ...
  try {
    ...
    await expect(page.getByTestId("kids-corner-heading")).toBeVisible({
      timeout: 120000,
    });
  } catch (e) {
    ...
    if (!(await littleSproutsNav.count()))
      littleSproutsNav = page.getByRole("button", {
        name: /Child Dashboard|Child|Little Sprouts|Willow's/i,
      });
    if (!(await littleSproutsNav.count())) {
      ...
      if (!(await commandCenterNav.count()))
        commandCenterNav = page.getByRole("button", { name: "The Workshop" });
      if (await commandCenterNav.count()) {
        ...
        if (systemBtnCount > 0) {
          ...
          const littleMenuCount = await page
            .getByRole("menuitem", { name: /Little Sprouts/i })
            .count();
          if (littleMenuCount > 0) {
            await retryClick(
              page.getByRole("menuitem", { name: /Little Sprouts/i }),
              { tries: 3 },
            );
          } else {
            await retryClick(page.getByText(/Little Sprouts/i), { tries: 3 });
  ...
  const fallbackSystemMenuNav = async () => {
    ...
    if (systemBtnCount > 0) {
      ...
      const menuItem = page.getByRole("menuitem", {
        name: /Child Dashboard|Child|Willow's Sprout|Willow's|Little Sprouts/i,
      });
  ...
  try {
    ...
    await page.waitForSelector('[data-testid="kids-reward-store"]', {
      state: "attached",
      timeout: 30000,
    });
    await expect(page.getByTestId("kids-reward-store")).toBeVisible({
      timeout: 15000,
    });
    ...
    let nav = page.getByRole("button", {
      name: /Child Dashboard|Child|Willow's Sprout|Willow's/i,
    });
  ...
  if (!dashboardLoaded) {
    ...
    try {
      ...
      throw new Error(
        "Child dashboard not loaded: Reward Store or Dopamine Mining not visible",
      );
    }
  }
  ...
  try {
    if (shouldTrace)
      await page.context().tracing.stop({ path: "trace-kids_rewards.zip" });
  } catch (e) {
