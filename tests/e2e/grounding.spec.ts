...
test.describe("GroundingRose E2E", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      try {
        (window as any).__E2E_HAPTICS_STUB__ = true;
      } catch (e) {
        /* ignore */
      }
    ...
    try {
      await page
        .getByRole("heading", {
          name: /Day Progress|Grounding|Temporal Status/i,
        })
        .first()
        .waitFor({ state: "visible", timeout: 15000 });
    } catch (e) {
      /* ignore — fallback below will handle slow pages */
    }
    await page.waitForLoadState("networkidle");
  });

  test("should trigger haptic feedback and visual ripple on activation", async ({
    page,
  }) => {
    ...
    const rippleCount = await page
      .locator('[data-testid="grounding-rose-ripple"]')
      .count();
    console.log("PW_DEBUG_RIPPLE_COUNT", rippleCount);
    const debugRose = await page.evaluate(() => {
      const nodes = Array.from(
        document.querySelectorAll('[data-testid="grounding-rose-button"]'),
      );
      return {
        count: nodes.length,
        tags: nodes.map((n) => n.tagName),
        pointerEvents: nodes.length
          ? getComputedStyle(nodes[0]).pointerEvents
          : null,
        disabled: nodes.length
          ? ((nodes[0] as HTMLButtonElement).disabled ?? null)
          : null,
      };
    });
    ...
    await expect(page.getByTestId("grounding-rose-ripple")).toHaveClass(
      /animate-ping/i,
      { timeout: 15000 },
    );
  ...
  test("should work without vibration support", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      try {
        (window as any).__E2E_HAPTICS_STUB__ = false;
      } catch (e) {
        /* ignore */
      }
      try {
        (window as any).hapticCalls = (window as any).hapticCalls || [];
      } catch (e) {
        /* ignore */
      }
    });
    ...
    const rippleVisible = await page
      .getByTestId("grounding-rose-ripple")
      .isVisible()
      .catch(() => false);
    const btnVisible = await page
      .getByTestId("grounding-rose-button")
      .isVisible();
