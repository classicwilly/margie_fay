  ...
  page: async ({ page, storageKey }, use) => {
    ...
    page.on("pageerror", async (exception) => {
      ...
      try {
        ...
        const png = path.join(
          dir,
          `${test.info().title.replace(/\s+/g, "_")}-pageerror-${time}.png`,
        );
        const htmlp = path.join(
          dir,
          `${test.info().title.replace(/\s+/g, "_")}-pageerror-${time}.html`,
        );
        ...
        console.error("Failed to save diagnostic snapshot on pageerror", err);
      }
    });
    page.on("console", (msg) => {
      ...
      if (msg.type() === "error" || msg.type() === "warn") {
        console.log(
          `PW_CONSOLE_LOG [${test.info().title}][${msg.type()}]:`,
          msg.text(),
        );
      }
    });
    ...
    page.on("close", () =>
      console.log(`PW_PAGE_CLOSE [${test.info().title}] page closed`),
    );
    page.on("crash", async () => {
      ...
      try {
        ...
        const png = path.join(
          dir,
          `${test.info().title.replace(/\s+/g, "_")}-crash-${time}.png`,
        );
        const htmlp = path.join(
          dir,
          `${test.info().title.replace(/\s+/g, "_")}-crash-${time}.html`,
        );
        await page.screenshot({ path: png, fullPage: true });
        const html = await page.content();
        fs.writeFileSync(htmlp, html, "utf-8");
        ...
        console.error("Failed to save diagnostic snapshot on crash", err);
      }
    });
        ...
        try {
          ...
          if (post) {
            ...
            if (typeof sys === "string") {
              if (sys.includes("William") || sys.includes("Grandpa"))
                sig = "Love, Grandpa.";
              else if (sys.includes("Bob Haddock") || sys.includes("—Bob"))
                sig = "—Bob.";
              else if (sys.includes("Marge Watson") || sys.includes("—Marge"))
                sig = "—Marge.";
            }
          }
        } catch (e) {
