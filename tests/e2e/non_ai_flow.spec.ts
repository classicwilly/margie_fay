...
test('app works without AI - manual flow @smoke', async ({ page, storageKey }) => {
  ...
  await page.goto('/?force_e2e_view=cockpit', { timeout: 120_000, waitUntil: 'load' });
  ...
  // Navigate to the weekly review
    ...
    const navCockpit = page.getByTestId('nav-cockpit').first();
    await expect(navCockpit).toBeVisible({ timeout: 10000 });
    await navCockpit.scrollIntoViewIfNeeded();
    await retryClick(navCockpit, { tries: 3, interval: 200, clickOptions: { force: true, position: { x: 5, y: 5 }, timeout: 10000 } });
  ...
  let proceedInboxBtn = page.getByTestId('weekly-review-proceed-inbox');
