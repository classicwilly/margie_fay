...
import { OscilloscopeProvider, useOscilloscope } from '@contexts/OscilloscopeContext';
...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <button data-testid="btn-deep" onClick={() => dispatch({ type: 'SET_FOCUS_MODE', payload: 'laser' })}>deep</button>
      <button data-testid="btn-neutral" onClick={() => dispatch({ type: 'SET_FOCUS_MODE', payload: 'none' })}>neutral</button>
      <button data-testid="btn-scout" onClick={() => dispatch({ type: 'SET_FOCUS_MODE', payload: 'scout' })}>scout</button>
    </div>
  );
}
