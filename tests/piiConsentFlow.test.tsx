...
vi.mock('@google/genai', () => ({
  GoogleGenAI: function GoogleGenAI() { return { models: { generateContent: vi.fn().mockResolvedValue({ text: 'ok from ai' }) } }; }
}));
...
describe('PII and Consent modal flow', () => {
  ...
  it('shows PII modal, then consent modal, then runs AI', async () => {
    ...
    const input = screen.getByTestId ? screen.getByTestId('ask-ai-input') : screen.getByPlaceholderText(/Describe the chaos/i);
    fireEvent.change(input, { target: { value: 'Contact me at test@example.com' } });

    const button = screen.getByTestId ? screen.getByTestId('ask-ai-btn') : screen.getByRole('button', { name: /Ask AI/i });
    fireEvent.click(button);

    // PII modal should appear
    await waitFor(() => expect(screen.getByText(/Potential Sensitive Data Detected!/i)).toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.getByText(/AI Feature Consent/i)).toBeInTheDocument());

    // Click 'Acknowledge & Proceed'
    const acknowledge = screen.getByRole('button', { name: /Acknowledge & Proceed/i });
