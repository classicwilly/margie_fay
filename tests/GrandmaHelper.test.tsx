...
describe('GrandmaHelper persona chooser', () => {
  ...
  it('asks Grandma by default and shows response', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/GRANDMA: How do I fix the sink/)).toBeInTheDocument());
  });
  ...
  it('switches to Grandpa and uses his persona', async () => {
    ...
    fireEvent.click(screen.getByTestId('grandma-ask-button'));
    await waitFor(() => expect(screen.getByText(/GRANDPA: How do I prune roses/)).toBeInTheDocument());
  });
  ...
  it('listens for the generic set-ai-query event and processes it by current persona', async () => {
    ...
    act(() => window.dispatchEvent(new CustomEvent('set-ai-query', { detail: { value: 'How do I prune roses?' } })));
    const input = screen.getByTestId('grandma-input');
    await waitFor(() => expect((input as HTMLInputElement).value).toBe('How do I prune roses?'));
    fireEvent.click(screen.getByTestId('grandma-ask-button'));
    await waitFor(() => expect(screen.getByText(/GRANDPA: How do I prune roses/)).toBeInTheDocument());
  });
  ...
  it('switches to Bob and uses Bob persona', async () => {
    ...
    fireEvent.click(screen.getByTestId('grandma-ask-button'));
    await waitFor(() => expect(screen.getByText(/BOB: How do I fix the fence/)).toBeInTheDocument());
  });
  ...
  it('switches to Marge and uses Marge persona', async () => {
    ...
    const input = screen.getByTestId('grandma-input');
    fireEvent.change(input, { target: { value: 'How do I organize my pantry?' } });
    fireEvent.click(screen.getByTestId('grandma-ask-button'));
    await waitFor(() => expect(screen.getByText(/MARGE: How do I organize my pantry/)).toBeInTheDocument());
  });
});
