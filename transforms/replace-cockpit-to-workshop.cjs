...
const IGNORE = ['node_modules', 'dist', '.git', 'coverage', 'playwright-report', 'tests/e2e'];
...
function walk(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      content = content.replace(/view\s*:\s*('|")cockpit('|")/g, (m, q1, q2) => `view: ${q1}workshop${q2}`);
      // 2) Replace view === 'cockpit' -> view === 'workshop'
      content = content.replace(/(===|==)\s*('|")cockpit('|")/g, (m, op, q1, q2) => `${op} ${q1}workshop${q2}`);
      // 3) Replace strings 'cockpit' -> 'workshop' when used as a literal in quotes, not in file names or import paths
      content = content.replace(/('|")cockpit('|")/g, (m, q1, q2) => `${q1}workshop${q2}`);
      if (content !== original) {
