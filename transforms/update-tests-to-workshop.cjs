  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      content = content.replace(
        /getByTestId\(("|')cockpit-([\w-\d${}.]+)("|')\)/g,
        (m, q1, id, q2) => `getByTestId(${q1}workshop-${id}${q2})`,
      );
      content = content.replace(
        /page.getByTestId\(("|')cockpit-([\w-\d${}.]+)("|')\)/g,
        (m, q1, id, q2) => `page.getByTestId(${q1}workshop-${id}${q2})`,
      );
      content = content.replace(
        /\[data-testid=("|')cockpit-([\w-]+)\1\]/g,
        (m, q, id) => `[data-testid="workshop-${id}"]`,
      );
