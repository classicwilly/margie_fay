  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      content = content.replace(
        /page\.getByTestId\((['\"])cockpit-([\w\d${}.-]+)\1\)/g,
        (m, q1, id) => {
          return `page.locator(byWorkshopOrCockpitTestId('workshop-${id}'))`;
        },
      );
      content = content.replace(
        /page\.getByTestId\((['\"])workshop-([\w\d${}.-]+)\1\)/g,
        (m, q1, id) => {
          return `page.locator(byWorkshopOrCockpitTestId('workshop-${id}'))`;
        },
      );
      ...
      content = content.replace(
        /page\.\$(\(['\"])\[data-testid=\\\"cockpit-([\w-]+)\\\"\]\\\1\)/g,
        (m, q1, id) => {
          return `page.locator(byWorkshopOrCockpitTestId('workshop-${id}'))`;
        },
      );
      ...
      content = content.replace(
        /\[data-testid=\"nav-cockpit\"\]/g,
        `\[data-workshop-testid=\"nav-workshop\"\], [data-testid=\"nav-cockpit\"]`,
      );

      // Ensure the helper import is present when we replaced getByTestId / $ with locator helper
      if (
        content !== original &&
        content.includes("byWorkshopOrCockpitTestId")
      ) {
