...
if (process.env.SENTRY_DSN) {
  try {
    ...
    console.warn("Sentry init failed for functions", e?.message || e);
  }
}
...
exports.aiProxy = functions.https.onRequest(async (req, res) => {
  ...
  const aiStubEnv =
    process.env.AI_STUB === "true" || process.env.PLAYWRIGHT_AI_STUB === "true";
  const notProd = process.env.NODE_ENV !== "production";
  const configAllow =
    functions.config &&
    functions.config().ai &&
    functions.config().ai.allow_stub === "true";
  ...
  const appCheckEnforce =
    ...
    (functions.config &&
      functions.config().appcheck &&
      functions.config().appcheck.enforce === "true");
  if (!appCheckToken) {
    if (appCheckEnforce) {
  ...
  try {
    ...
    const aiKey =
      ...
      (functions.config && functions.config().ai && functions.config().ai.key);
    const geminiModel =
    ...
    const aiUrl =
      ...
      (functions.config &&
        functions.config().ai &&
        functions.config().ai.url) ||
      `https://generativeai.googleapis.com/v1/models/${geminiModel}:generate`;
    ...
    if (
      !aiKey &&
      fallbackGeminiKey &&
      aiUrl.includes("generativeai.googleapis.com")
    ) {
